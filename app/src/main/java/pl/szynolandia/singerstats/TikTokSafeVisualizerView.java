...
public class TikTokSafeVisualizerView extends VisualizerView {
    ...
    static void drawSafeScene(Canvas c,ProjectData pr,double t){
        ...
        p.setTextAlign(Paint.Align.CENTER); p.setColor(Color.WHITE); p.setFakeBoldText(true); p.setTextSize(52); c.drawText("KTO ILE ŚPIEWA?",495,285,p);
        String title=(pr.songTitle==null||pr.songTitle.trim().isEmpty())?"Bez nazwy utworu":pr.songTitle.trim();
        p.setTextSize(40); c.drawText(title,495,342,p);
        p.setFakeBoldText(false); p.setTextSize(27); p.setColor(Color.LTGRAY); double dur=Math.max(pr.songDuration,t); c.drawText(VisualizerView.fmt(t)+" / "+VisualizerView.fmt(dur),495,382,p);
        ...
        if(active!=null){
            ...
            p.setTextAlign(Paint.Align.LEFT); p.setTextSize(31); p.setFakeBoldText(true);
            ...
            p.setFakeBoldText(false); p.setColor(Color.WHITE); p.setTextSize(39); VisualizerView.wrap(c,p,active.text,left+30,635,width-60,43,2);
        }

        p.setTextAlign(Paint.Align.CENTER); p.setColor(Color.WHITE); p.setTextSize(31); p.setFakeBoldText(true); c.drawText("STATYSTYKI CZASU",495,815,p); p.setFakeBoldText(false);
        ...
        for(Singer s:pr.singers){
            ...
            p.setTextAlign(Paint.Align.LEFT); p.setTextSize(26); p.setFakeBoldText(true); p.setColor(Color.WHITE); c.drawText(rank+". "+s.name,left+76,y,p);
            p.setFakeBoldText(false); p.setTextSize(23); p.setTextAlign(Paint.Align.RIGHT); c.drawText(String.format(Locale.US,"%.1fs • %.1f%%",sec,pct*100),left+770,y,p);
